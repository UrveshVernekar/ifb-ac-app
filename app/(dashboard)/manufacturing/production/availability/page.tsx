// app/(dashboard)/manufacturing/production/availability/page.tsx
"use client";
...
interface ApiResponse {
    ...
    oeeTarget?: number;
    availability?: number;
    dailyAvailability?: DailyAvailabilityItem[];
}
...
export default function AvailabilityDashboardPage() {
    ...
    // API states
    const [oeeData, setOeeData] = useState<ApiResponse | null>(null);
    const [downtimeData, setDowntimeData] = useState<DowntimeLogItem[]>([]);
    ...
    const paginatedDowntimeData = useMemo(() => {
        return downtimeData.slice(startIndex, startIndex + pageSize);
    }, [downtimeData, startIndex, pageSize]);
    ...
    // Fetch availability and downtime details
    const fetchData = async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : "Failed to fetch availability metrics.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="space-y-6 max-w-7xl mx-auto p-2">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1">
                            ...
                            Downtime event histories and run statistics
                        </p>
                    </div>
                </div>
            ...
            {loading && !oeeData ? (
                ...
                oeeData && (
                    <div className="space-y-6">
                        {/* Overall card */}
                        {oeeData.availability !== undefined && (
                            <Card className={`border shadow-sm max-w-sm ${getOeeColor(oeeData.availability, 90)}`}>
                                <CardHeader className="pb-1 pt-4 text-center">
                                    <CardTitle className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Availability Score</CardTitle>
                                </CardHeader>
                                <CardContent className="pb-4 pt-1 flex flex-col items-center justify-center">
                                    <div className="text-4xl font-extrabold text-foreground">{Number(oeeData.availability).toFixed(1)}%</div>
                        ...
                        {oeeData.dailyAvailability && oeeData.dailyAvailability.length > 0 && (
                            <Card className="border border-border/60 shadow-sm bg-card">
                                <CardHeader className="pb-2">
                                    ...
                                    <CardDescription className="text-xs">Availability fluctuations over selected days</CardDescription>
                                </CardHeader>
                                <CardContent className="pt-2">
                                    <div className="h-72 w-full">
                                        <ChartContainer config={{}} className="h-full w-full">
                                            <BarChart data={oeeData.dailyAvailability} margin={{ top: 20, right: 10, left: -10, bottom: 0 }}>
                                                ...
                                                <Bar dataKey="value" name="Availability" fill="#ef4444" radius={[3, 3, 0, 0]}>
                                                    {oeeData.dailyAvailability.map((entry, index) => {
                        ...
                        <Card className="border border-border/60 shadow-sm bg-card">
                            <CardHeader className="pb-2">
                                ...
                                <CardDescription className="text-xs">List of downtime instances recorded during timeframe</CardDescription>
                            </CardHeader>
                            <CardContent className="pt-2">
                                {downtimeData.length > 0 ? (
                                    <div className="overflow-x-auto">
                                        <Table>
                                            <TableHeader>
                                                <TableRow>
                                                    ...
                                                    <TableHead className="text-xs font-bold uppercase tracking-wider">Line</TableHead>
                                                    <TableHead className="text-xs font-bold uppercase tracking-wider">Type</TableHead>
                                                    <TableHead className="text-xs font-bold uppercase tracking-wider">Reason</TableHead>
                                                    <TableHead className="text-xs font-bold uppercase tracking-wider text-right">Duration (min)</TableHead>
                                                </TableRow>
                                            </TableHeader>
                                            <TableBody>
                                                {paginatedDowntimeData.map((row, rIdx) => (
                                                    <TableRow key={rIdx}>
                                                        ...
                                                        <TableCell className="text-xs py-2 max-w-[200px] truncate">{row.reason || row.REASON || ""}</TableCell>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-4 pt-4 border-t text-xs">
                                            <span className="text-xs text-muted-foreground">
                                                Showing {downtimeData.length > 0 ? startIndex + 1 : 0} to {Math.min(startIndex + pageSize, downtimeData.length)} of {downtimeData.length} entries
                                            </span>
                                    ...
                                    <div className="text-center py-6 text-xs text-muted-foreground">
                                        No downtime logs recorded in this period.
                                    </div>
                                )}
                            </CardContent>
                        </Card>
